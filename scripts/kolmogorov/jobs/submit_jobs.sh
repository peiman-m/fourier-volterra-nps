...
for BASE in "${BASES[@]}"; do
    for MODEL_TYPE in "${MODEL_TYPES[@]}"; do
        EXPERIMENT_NAME="kolmogorov/default"
        HYDRA_OVERRIDES="model/kolmogorov=${MODEL_TYPE} ${EXTRA_HYDRA_OVERRIDES}"
        ...
        for SEED in "${SEED_VALUES[@]}"; do
            ...
            if [[ "$DO_EVAL" == "true" ]]; then
                ...
                # eval-only: W&B off (metrics still saved locally to artifacts/.../metrics/)
                EVAL_HYDRA_OVERRIDES="${HYDRA_OVERRIDES} misc.wandb_logging_enabled=false"
