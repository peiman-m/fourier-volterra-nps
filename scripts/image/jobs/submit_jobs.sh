...
# Extra Hydra overrides appended to every run. Eval-only run: disable W&B
# (metrics still saved locally to metrics_dirpath). Clear for full training.
EXTRA_HYDRA_OVERRIDES="misc.wandb_logging_enabled=false"
...
# Base-Datatype pairs: Format is "data_image_group:label"
# The first field is the Hydra ``benchmark/image`` group member (see
# ``conf/benchmark/image/``); the second is the display label used in job
# names. Example: "base-cifar10:CIFAR10".
BASE_DATATYPE_PAIRS=(
    "base-cifar10:CIFAR10"
    # "base-mnist:MNIST"      ## --> Don't forger to fix the likelihood
    "base-svhn:SVHN"
    # "base-dtd:DTD"
)
...
MODEL_TYPES=(
    ...
    "convcnp-resnet"
    "convcnp-fno"
    "sf-convcnp-f4.8-e384"
    "sf-volterra-convcnp-f4.75-e256-l6-vr2"
)
...
for PAIR in "${BASE_DATATYPE_PAIRS[@]}"; do
    BASE="${PAIR%%:*}"
    DATA_TYPE="${PAIR##*:}"

    for MODEL_TYPE in "${MODEL_TYPES[@]}"; do
        EXPERIMENT_NAME="image/default"
        HYDRA_OVERRIDES="model/image=${MODEL_TYPE} benchmark/image=${BASE} ${EXTRA_HYDRA_OVERRIDES}"
        ...
        for SEED in "${SEED_VALUES[@]}"; do
            ...
            if [[ "$DO_EVAL" == "true" ]]; then
                CUSTOM_JOB_NAME="eval_${MODEL_TYPE}_${DATA_TYPE}_s${SEED}"
                DEPENDENCY=""
                ...
                if ! sbatch_job "$JOB_FILE" \
                    "$EXPERIMENT_NAME" "$HYDRA_OVERRIDES" "$SEED" "eval" "$ACCOUNT" \
