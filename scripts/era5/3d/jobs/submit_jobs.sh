...
# Eval Region Configuration (only consumed when DO_EVAL=true)
# Each entry must match a YAML under conf/benchmark/era5/eval_region/<R>.yaml.
# Each region produces an independent eval job: separate W&B run
# (<model>-seed=<s>_eval_<region>) and separate local metrics dir
# (artifacts/.../seed=<s>/metrics/<region>/), so metrics never collide.
EVAL_REGIONS=("center" "west" "north")

# Train SLURM resources
TRAIN_TIME="10:00:00"
TRAIN_NODES=1
TRAIN_NTASKS_PER_NODE=1
TRAIN_CPUS_PER_TASK=20
TRAIN_MEM="150G"
...
declare -A CLUSTER_ACCOUNTS=(
    ...
    ["LAUNCH"]="YOUR_LAUNCH_ACCOUNT"   # Launch cluster
)
...
MODEL_TYPES=(
    ...
    # "convcnp-resnet"
    # "convcnp-fno"
    "sf-convcnp-f4.25-e176"
    "sf-volterra-convcnp-f3.75-e52-l6-vr2"
)
...
for BASE in "${BASES[@]}"; do
    for MODEL_TYPE in "${MODEL_TYPES[@]}"; do
        EXPERIMENT_NAME="era5/default"
        HYDRA_OVERRIDES="model/era5=${MODEL_TYPE} ${EXTRA_HYDRA_OVERRIDES}"
        ...
        for SEED in "${SEED_VALUES[@]}"; do
            ...
            if [[ "$DO_TRAIN" == "true" ]]; then
                ...
                TRAIN_JOB_ID=$(sbatch_job "$JOB_FILE" \
                    "$EXPERIMENT_NAME" "$HYDRA_OVERRIDES" "$SEED" "train" "$ACCOUNT" \
            ...
            if [[ "$DO_EVAL" == "true" ]]; then
                for REGION in "${EVAL_REGIONS[@]}"; do
                    CUSTOM_JOB_NAME="eval_${MODEL_TYPE}_${DATA_TYPE}_s${SEED}_${REGION}"
                    DEPENDENCY=""
                    [[ -n "$TRAIN_JOB_ID" ]] && DEPENDENCY="afterok:$TRAIN_JOB_ID"
                    REGION_OVERRIDES="+benchmark/era5/eval_region=${REGION}"
                    # eval-only: W&B off (metrics still saved locally to artifacts/.../metrics/<region>/)
                    EVAL_HYDRA_OVERRIDES="${HYDRA_OVERRIDES} ${REGION_OVERRIDES} misc.wandb_logging_enabled=false"
                    echo "Submitting eval job: $CUSTOM_JOB_NAME${DEPENDENCY:+ (depends on job $TRAIN_JOB_ID)}"
                    if ! sbatch_job "$JOB_FILE" \
                        "$EXPERIMENT_NAME" "$EVAL_HYDRA_OVERRIDES" "$SEED" "eval" "$ACCOUNT" \
