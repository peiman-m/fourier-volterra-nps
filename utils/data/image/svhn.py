...
class SVHNDataProcessor(torch.utils.data.Dataset):
    """Street View House Numbers (SVHN) dataset processor.
    ...
    Args:
        ...
        split: One of ``"train"``, ``"val"``, ``"test"``, or ``"extra"``.
    ...
    def __init__(
        ...
        **kwargs,
        ...
        if split not in ("train", "val", "test", "extra"):
            raise ValueError(
                f"split must be 'train', 'val', 'test', or 'extra', got '{split}'"
            )
...
class SVHNDataset(ImageDataset):
    """DataLoader for SVHN image datasets."""

    def __init__(self, *, processor: SVHNDataProcessor, **kwargs: Any) -> None:
        if not isinstance(processor, SVHNDataProcessor):
            raise TypeError(f"Expected SVHN processor, got {type(processor).__name__}")
