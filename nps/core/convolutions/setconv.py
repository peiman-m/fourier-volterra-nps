...
class BaseSetConv(BaseConvolution, ABC):
    ...
    def __init__(
        ...
        *,
        ...
        Args:
            ...
            density_feature_pairing: Output channel layout. "blocked"
                (default) concatenates [D-block | F-block]. "interleaved"
                pairs each density channel with its feature channel,
                (D_1, F_1, D_2, F_2, ...), and requires density and features
                to have the same channel count.
        """
        super().__init__(
        ...
        if density_feature_pairing not in ("blocked", "interleaved"):
            raise ValueError(
                f"density_feature_pairing must be 'blocked' or 'interleaved', "
    ...
    def _process_output(
        ...
        Returns:
            Density and normalized feature channels combined per
            `density_feature_pairing`: [D-block | F-block] for "blocked"
            (default), (D_1, F_1, D_2, F_2, ...) for "interleaved".
        """
        ...
        # "interleaved": (D_1, F_1, D_2, F_2, ...) — needs matching widths.
        if z_density.shape != zq_updated.shape:
            raise ValueError(
                f"interleaved pairing requires density shape "
                f"{tuple(z_density.shape)} to match feature shape "
                f"{tuple(zq_updated.shape)}"
            )
        pair = [z_density, zq_updated]
        if channels_dim in (-1, zq_updated.ndim - 1):
            return einops.rearrange(pair, "two ... c -> ... (c two)")
        if channels_dim == 1:
            return einops.rearrange(pair, "two b c ... -> b (c two) ...")
        raise ValueError(f"Unsupported channels_dim: {channels_dim}")
    ...
    def __init__(
        ...
        *,
        ...
        Args:
            ...
            density_feature_pairing: Output layout, "blocked" (default,
                original concatenation) or "interleaved"; see BaseSetConv.
        """
        super().__init__(
            spatial_dim=spatial_dim,
            in_channels=in_channels,
...
class GridSetConv(BaseSetConv):
    ...
    def __init__(
        ...
        *,
        ...
        Args:
            ...
            in_channels: Number of input feature channels
            convnet: CNN applied to both the feature and density grids
                (defaults to Identity). A single shared module is deliberate —
                analogous to SetConv smoothing density and features with the
                same lengthscales. The mask is expanded to in_channels before
                the convnet so both passes see the same input width.
            divide_by_density: Whether to normalize output by density
            epsilon: Numerical stabilizer for division by zero
            density_feature_pairing: Output layout, "blocked" (default,
                [D-block | F-block]) or "interleaved"; see BaseSetConv.
        """
        convnet = convnet or nn.Identity()

        # Determine out_channels
        if hasattr(convnet, "out_channels"):
            out_channels = 2 * getattr(convnet, "out_channels")
        elif isinstance(convnet, nn.Identity):
            out_channels = 2 * in_channels
        else:
            raise ValueError(
    ...
    def forward(
        ...
        *,
        ...
        Returns:
            Output tensor of shape (B, 2 * in_channels, ...) combining density
            and value channels per `density_feature_pairing`. A single-channel
            mask is expanded to feature width, so the output is always
            2 * in_channels wide.
        """
        ...
        # Expand a shared 1-channel mask to feature width so the single shared
        # convnet processes density and features with the same input channels.
        xkv_mask = xkv_mask.to(zv.dtype)
        if xkv_mask.shape[1] == 1:
            xkv_mask = einops.repeat(xkv_mask, "b 1 ... -> b c ...", c=zv.shape[1])

        # Apply the shared convnet to both grids
        zq_updated = self.convnet(zv * xkv_mask)
        z_density = self.convnet(xkv_mask)
